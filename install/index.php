...
if ($_POST) {
    ...
    if ($core->validate_post($_POST) == true) {
        ...
        if (!isset($message)) {
            ...
            header('Location: ' . $redir . 'register/user');
        }

    } else {
        $message = $core->show_message(
            'error',
...
<?php if (is_writable($db_config_path)) { ?>
    ...
    <form id="install_form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <fieldset>
            ...
            <label for="hostname">Hostname</label><input type="text" id="hostname" value="localhost" class="input_text"
                                                         name="hostname"/>
            <label for="username">Username</label><input type="text" id="username" class="input_text" name="username"/>
            <label for="password">Password</label><input type="password" id="password" class="input_text"
                                                         name="password"/>
            <label for="database">Database Name</label><input type="text" id="database" class="input_text"
                                                              name="database"/>
        </fieldset>

        <fieldset>
            <legend>Javascript settings</legend>
            <label for="">Nodejs Server</label><input name="nodejs" type="text"
                                                      placeholder="http://pubsweet.local:8080/"/>
            <label for="">Server Name</label><input name="servername" type="text" placeholder="http://pubsweet.local/"/>
        </fieldset>
