...
class Auth extends CI_Controller
{
	...
	function login()
	{
		if ( ! $this->dx_auth->is_logged_in())
		{
			...
			{
				$this->log($val->set_value('username'));
...
//						$this->dx_auth->captcha();
					...
					$this->load->view($this->dx_auth->login_view, array('error'=>true));
...
//				$this->load->view($this->dx_auth->register_success_view, $data);
                $this->dx_auth->login(($this->input->post('username')),($this->input->post('password')), false);
                $this->load->model('inviteds_model');
                if($this->inviteds_model->checkInvitations($this->input->post('mail'))){
                    $this->load->library('Mail');
                    $this->mail->send('pubsweet-admin@booksprints.net', 'This guy has just accepted',
                        'A new invited user has just accepted his invitation');
                }

                $this->session->set_flashdata('register', 1);
				redirect($this->dx_auth->register_success_uri, 'refresh');
			}
			else
			{
