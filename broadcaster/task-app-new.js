...
var pubsweet = io
  ...
  .on('connection', function (socket) {
        ...
        socket.on('editing-books',function(data){
            var users= {},repeated=false;
            var isNew = true;
            var length =0;
            for(var i in data)
                length++;
            if(length >= 4){
                for(var i in usersConnect){
                    repeated = false;
                    for(var j in usersConnect){
                        if(JSON.stringify(usersConnect[i].id) == JSON.stringify(usersConnect[j].id)){
                            if(!repeated){
                                repeated = true;
                                if(JSON.stringify(data.id) != JSON.stringify(usersConnect[j].id)){
                                    users[j] = usersConnect[i];
                                }
                            }
                        }
                    }
                    if(JSON.stringify(usersConnect[i].id) ==JSON.stringify(data.id)){
                        isNew = false;
                    }
                }
                socket.emit('users-editing-book',users);
                if (isNew){
...
//        });
        ...
        socket.on('disconnect',function(){
            ...
            var length =0;
            for(var i in usersConnect[socket.id])
                length++;
            if(length >=4){
                var otherSession = false;
                for(var item in usersConnect){
                    if(item != socket.id){
                    if(JSON.stringify(data.id) == JSON.stringify(usersConnect[item].id)){
                        otherSession = true;
                        break;
                    }
                    }
